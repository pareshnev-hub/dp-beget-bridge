...
// This reserves space for existing DB bytes copied through recovery, the
...
export async function inspectStateBundleSpace({ databases, parent,
  ...
  for (const database of databases) {
    ...
    const info = await inspectFile(database.source);
    if (!info.isFile() || info.nlink !== 1 || !Number.isSafeInteger(info.size) || info.size < 0) {
      throw new Error("State bundle database source cannot be sized safely");
    }
    databaseBytes += BigInt(info.size);
  }
  ...
  const requiredBytes = COPIES * (databaseBytes + MAX_CONFIG_BYTES) + MIN_FREE_AFTER;
  if (availableBytes < requiredBytes) {
    throw new Error("Insufficient free space for grouped migration snapshot and recovery copies");
  }
  return { availableBytes, requiredBytes, databaseBytes,
