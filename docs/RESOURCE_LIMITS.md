...
- `DP_TRANSCRIPT_SEGMENT_BYTES` defaults to 8 MiB; it divides new transcripts into bounded files under the same per-session and aggregate limits. Existing single-file transcripts remain readable. R0004 retains local transcripts until explicit purge. A CLOSED transcript can be explicitly copied to a private, checksummed offline archive; the copy occupies local disk independently of Session Host's transcript quota. R0004 never deletes archives or transcripts automatically by age.
