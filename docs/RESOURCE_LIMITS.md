...
- `DP_TRANSCRIPT_SEGMENT_BYTES` defaults to 8 MiB; it divides new transcripts into bounded files under the same per-session and aggregate limits. Existing single-file transcripts remain readable. The segmented files are retained until explicit purge; automatic archival and age-based retention still require a separate policy.
