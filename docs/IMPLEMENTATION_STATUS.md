...
| Local release and resource diagnostics | `5b37249` (#211) | Local doctor identifies a managed release version/commit, checks state schema version read-only and reports the disk reserve; signing and database integrity remain separate release gates |
| Combined signed Beget preflight | this change | One read-only entrypoint joins pinned signature, host profile, current R0003 route/data bindings, shared-volume space and exclusive-route checks; awaits a production key, signed candidate and actual Beget execution. No drain or migration authorization |
