...
SQLite restore must account for `-wal`/`-shm` sidecars and ownership of each service's state directory. A multi-file restore without a crash-recoverable journal can leave stores from different phases mixed. The current `restore-state-bundle.mjs` only recreates data in a **new** directory; live state replacement is not implemented. Existing unit backup covers the four bridge fragments and OAuth drop-in, but **does not yet** cover dedicated proxy/tunnel unit files or boot-guard overrides. No public update or N/N-1 rollback gate is accepted until these parts and a disposable systemd migration/failed-update test pass, followed by exact-commit Beget evidence.
